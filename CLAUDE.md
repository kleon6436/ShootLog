...
`integration.` `slideshow.` `toast.` `crop.` `upscale.` `develop.` `photosLibrary.` `ai.` のいずれかで始める。`a11y.` は表示ラベルをそのまま流用できない場合にだけ新設する。
...
- フォルダスキャン時にファイルサイズ・更新日時・作成日時を一括取得し、`FileAttributesSnapshot` としてメモリに保持する。プレビューキャッシュキーとEXIFのファイルサイズ判定はこのスナップショットを再利用し、取得できない場合だけ個別属性取得へフォールバックする。iCloud写真ライブラリの写真には適用しない。
- フォルダ内容の自動変更検知は行わない。外部変更の反映は、ユーザーがフォルダを再選択または履歴から再度開いた時の再スキャンで行う。

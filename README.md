...
- RAW現像編集（露出・コントラスト・トーンカーブ・カラー別HSL・ノイズ低減など。Core Imageベース、非破壊）
...
- 現像編集のカラー別HSLはCPU生成の3D LUTによる近似で、出力はsRGB固定です。RAW固有の現像パラメータは適用せず、`CIRAWFilter`はベースデコードのみに使います。トリミングは書き出し時にのみ反映されます。
...
フォルダ読み込み、履歴、EXIF表示、サムネイル/高解像度画像ロード、SwiftData永続化、お気に入り・メモ、3表示モード、非破壊編集、RAW現像編集とJPEG/TIFF書き出し、撮影設定分析、外部アプリ起動連携を実装しています。

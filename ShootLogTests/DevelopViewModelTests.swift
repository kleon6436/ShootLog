...
struct DevelopViewModelTests {
    ...
    final class SpyMaskGenerator: SubjectMaskGenerating, @unchecked Sendable {
        private let lock = NSLock()
        private var calls = 0
    ...
    private func makeMaskPNGData(longEdge: Int = 8) throws -> Data {
    ...
    private func makeMaskViewModelWithContent(
        engine: SpyEngine,
    ...
    private func aiRasterID(of layer: MaskLayer) throws -> UUID {
        guard case .ai(let reference) = layer.source else {
            Issue.record("AI ソースではない")
    ...
    private enum MaskRasterTestError: Error { case notAISource }

...
// トラックパッドの2本指水平スワイプを検知して SwiftUI 側へ通知する AppKit ブリッジ。
//
...
//
// 使用例（フルスクリーンモードでの写真切替）:
//
//     PhotoViewerView(photo: vm.selectedPhoto, editInfo: vm.currentEditInfo)
//         .background(
//             TrackpadSwipeCatcher(
//                 // ズーム中（scale > 1.0）はパンを優先し、fit 倍率の時だけ写真切替を有効にする
//                 isEnabled: zoomScale <= 1.0,
...
struct TrackpadSwipeCatcher: NSViewRepresentable {
    /// スワイプ検知の有効・無効。false の間はマウス・スクロールいずれのイベントも
    /// 消費せず完全に透過するため、ズーム中のパン処理などを妨げない
    ...
    final class SwipeCatchingView: NSView {
        ...
        var isEnabled: Bool = true {
            didSet {
                // 無効化された時点で蓄積中のスワイプは破棄する
                if !isEnabled { resetAccumulation() }
            }
        }
        ...
        override func scrollWheel(with event: NSEvent) {
            // 精密デルタを持たない通常のマウスホイールは phase を持たず
            // ロックを解除できないため対象外にする
            guard isEnabled,
                  event.hasPreciseScrollingDeltas,
                  !event.phase.isEmpty || !event.momentumPhase.isEmpty else {

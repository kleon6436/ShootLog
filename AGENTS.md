...
`integration.` `slideshow.` `toast.` `crop.` `upscale.` `develop.` `ai.` のいずれかで始める。`a11y.` は表示ラベルをそのまま流用できない場合にだけ新設する。
